 ...
 * @since 3.1
 */
class InformationDialog : JFrame(), Runnable {
    private val panes: JTabbedPane

    @Transient
    private val tabs = arrayOf<InformationTab>(
        FileFromSourceTab(
            "Program Properties",
            FileFromSourceTab.Companion.getContent(LegalNoticeButton.Companion.LEGAL_NOTICE_PROPERTIES_DOCS)
        ),
        FileFromSourceTab(
            "Legals",
            FileFromSourceTab.Companion.getContent(LegalNoticeButton.Companion.LEGAL_NOTICE_DOCS)
        ),
        SystemTab(),
        DebuggerTab()
    )
    ...
    init {
        ...
        panes = JTabbedPane()
        panes.tabLayoutPolicy = JTabbedPane.WRAP_TAB_LAYOUT
        for (tab in tabs) {
            panes.addTab(tab.name, tab.component)
        }
