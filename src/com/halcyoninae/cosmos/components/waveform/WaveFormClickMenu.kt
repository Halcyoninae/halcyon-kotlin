 ...
 * @since 3.2
 */
class WaveFormClickMenu(private val e: WaveFormPane) : MouseAdapter() {
    /**
     * @param ex
     */
    private fun attempt(ex: MouseEvent) {
        ...
        close.addActionListener { event_1: ActionEvent? -> e.isVisible = !e.isVisible }
        val stayTop = JMenuItem("Stay On Top")
        stayTop.addActionListener { event_1: ActionEvent? -> e.isAlwaysOnTop = !e.isAlwaysOnTop }
        val hideBC = JMenuItem("Hide Big Player")
        hideBC.addActionListener { event_1: ActionEvent? -> Halcyon.bgt.getFrame().isVisible = false }
        val showBC = JMenuItem("Show Big Player")
        showBC.addActionListener { event_1: ActionEvent? -> Halcyon.bgt.getFrame().isVisible = true }
        menu.add(close)
        menu.add(stayTop)
        menu.add(showBC)
        menu.add(hideBC)
