 ...
 * @since 3.2
 */
class WaveForm : JPanel(), FrameBufferListener {
    @Transient
    private val lock = Any()
    private var samples: FloatArray?
    private var path = Path2D.Float()
    private var s_valid = 0
    var isFakeVisible = false
        private set

    init {
        preferredSize = Dimension(WaveFormManager.Companion.MIN_WIDTH, WaveFormManager.Companion.MIN_HEIGHT)
        isOpaque = false
     ...
     * @param svalid
     */
    fun make(samples: FloatArray?, svalid: Int) {
    ...
    public override fun paintComponent(g: Graphics) {
        if (isFakeVisible) {
            if (Global.player.stream!!.audioFormatAbsolute != null) {
                s_valid /= 4
                val main = path
                var avg = 0f
                val hd2 = height / 2f
                val chnls = Global.player.stream!!.audioFormatAbsolute.channels
                var i = 0
                while (i < chnls && i < s_valid) avg += samples!![i++]
                avg /= chnls.toFloat()
                main.reset()
                main.moveTo(0f, hd2 - avg * hd2)
                val fvalid = s_valid / chnls
                var ch: Int
                var frame = 0
                while (i < s_valid) {
                    avg = 0f
                    ...
                    avg /= chnls.toFloat()
                    main.lineTo(
                        frame.toFloat() / fvalid * width, hd2 - avg * hd2
                    )
                    frame++
                }
                path = main
                ...
                synchronized(lock) {
                    g2.background = ColorManager.ONE_DARK_BG
                    ...
                    g2.paint = ColorManager.MAIN_FG_THEME
                    g2.draw(path)
                }
     ...
     * @param s_valid
     */
    override fun frameUpdate(samples: FloatArray?, s_valid: Int) {
        async {
    ...
    companion object {
        private val renderinghints: MutableMap<RenderingHints.Key?, Any?> = HashMap()
