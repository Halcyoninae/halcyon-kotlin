 ...
 * @since 3.2
 */
class MiniContentPane constructor() : JPanel(), InfoViewUpdateListener {
    private val bgPanel: JPanel
    private val fgPanel: JPanel
    private val topPanel: JPanel
    private val progressPanel: JPanel
    private val mainLabel: JLabel
    private val artLabel: JLabel
    private val progressBar: JProgressBar

    @Transient
    private var info: AudioInfo
    private var fDrawn: Boolean = true

    init {
        setPreferredSize(
            Dimension(
                MiniPlayerManager.Companion.MINI_PLAYER_MIN_WIDTH,
                MiniPlayerManager.Companion.MINI_PLAYER_MIN_HEIGHT
            )
        )
        ...
        bgPanel = object : JPanel() {
            public override fun paintComponent(g: Graphics) {
                ...
                if (info.hasArtwork() || !fDrawn) {
                    warn("Found a new artwork!@MINIPLAYER")
                    ...
                    var last: BufferedImage? = info.getArtwork()
                    last = resize((last)!!, getWidth(), getHeight())
                    g2.drawImage(
                        createGradient(
                            MiniDeImage.blurHash(
                                last, 5,
                                5
                            ), 255, 0, Directional.LEFT
                        ),
                        0, 0, null
                    )
                    g2.dispose()
                    ...
                    warn("Reusing last artwork!")
                }
            }

            public override fun isOptimizedDrawingEnabled(): Boolean {
                return false
            }
        }
        ...
        topPanel = JPanel()
        topPanel.setPreferredSize(Dimension(getPreferredSize().width, getPreferredSize().height - 15))
        topPanel.setLayout(GridLayout(1, 2))
        ...
        artLabel = JLabel(
            ImageIcon(createRoundedBorder(resizeNoDistort(info.getArtwork(), 128, 128), 10, null))
        )
        artLabel.setDoubleBuffered(true)
        topPanel.add(artLabel)
        topPanel.add(mainLabel)
        progressPanel = JPanel()
        progressPanel.setPreferredSize(Dimension(getPreferredSize().width, 15))
        fgPanel = JPanel()
        fgPanel.setPreferredSize(getPreferredSize())
        fgPanel.setLayout(BorderLayout())
        ...
        progressBar.setBackground(Color(0, 0, 0, 50))
        progressBar.setForeground(ColorManager.MAIN_FG_THEME)
        Thread(Runnable({
            while (true) {
                if (Global.player.stream!!.isPlaying()) {
                    if (Global.player.stream!!.getLength() > 0) {
                        progressBar
                            .setValue((Global.player.stream!!.getPosition() * progressBar.getMaximum() / Global.player.stream!!.getLength()).toInt())
                    } else {
    ...
    private val labelString: String
        private get() {
            return ("<html><p style=\"color:" + ColorManager.MAIN_FG_STR + ";font-size:12px\"><b>"
                    + (if (info.getTag(AudioInfo.KEY_MEDIA_TITLE).length > 28) info.getTag(AudioInfo.KEY_MEDIA_TITLE)
                .substring(0, 28) + "..." else info.getTag(AudioInfo.KEY_MEDIA_TITLE))
     ...
     * @param info
     */
    public override fun infoView(info: AudioInfo) {
        this.info = info
        scheduleRedraw()
    }
}