 ...
 * @since 3.2
 */
class MiniPlayerClickMenu constructor(private val e: MiniPlayer) : MouseAdapter() {
    /**
     * @param ex
     */
    private fun attempt(ex: MouseEvent) {
        val x: Int = ex.getX()
        val y: Int = ex.getY()
        val menu: JPopupMenu = JPopupMenu()
        menu.setBorder(BorderFactory.createEmptyBorder())
        val close: JMenuItem = JMenuItem("Close")
        close.addActionListener(ActionListener({ event_1: ActionEvent? -> e.setVisible(!e.isVisible()) }))
        val stayTop: JMenuItem = JMenuItem("Stay On Top")
        stayTop.addActionListener(ActionListener({ event_1: ActionEvent? -> e.setAlwaysOnTop(!e.isAlwaysOnTop()) }))
        val hideBC: JMenuItem = JMenuItem("Hide Big Player")
        hideBC.addActionListener(ActionListener({ event_1: ActionEvent? -> Halcyon.bgt.getFrame().setVisible(false) }))
        val showBC: JMenuItem = JMenuItem("Show Big Player")
        showBC.addActionListener(ActionListener({ event_1: ActionEvent? -> Halcyon.bgt.getFrame().setVisible(true) }))
        menu.add(close)
        menu.add(stayTop)
        menu.add(showBC)
        menu.add(hideBC)
