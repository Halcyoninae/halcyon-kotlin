 ...
 * @since 3.0
 */
class TopPane(ifp: InfoViewTP?, bctp: ButtonControlTP?) : JPanel() {
    val TOPPANE_MAX_WIDTH: Int = TOPPANE_MIN_WIDTH - FileList.Companion.FILEVIEW_MAX_WIDTH
    val TOPPANE_MAX_HEIGHT = Manager.MAX_HEIGHT / 2
    /// TopPane Config END
    /**
     ...
     */
    init {
        preferredSize = Dimension(TOPPANE_MIN_WIDTH, TOPPANE_MIN_HEIGHT)
        minimumSize = Dimension(TOPPANE_MIN_WIDTH, TOPPANE_MIN_HEIGHT)
        layout = BorderLayout()
    ...
    companion object {
        ...
        const val TOPPANE_MIN_WIDTH = Manager.MIN_WIDTH
        const val TOPPANE_MIN_HEIGHT = Manager.MIN_HEIGHT / 2
    }
}