 ...
 * @since 3.3
 */
class TimeControlSubTP : JPanel(), ActionListener {
    /// TimeControl Config START
    val FAST_FORWARD_ICON = Manager.RSC_FOLDER_NAME + "/time_control/forward.png"
    val FAST_BACKWARD_ICON = Manager.RSC_FOLDER_NAME + "/time_control/backward.png"
    val TO_END_ICON = Manager.RSC_FOLDER_NAME + "/time_control/end.png"
    val TO_START_ICON = Manager.RSC_FOLDER_NAME + "/time_control/start.png"
    val ICON_SIZING = 18
    val TIMECONTROL_HEIGHT: Int = (ButtonControlTP.Companion.BUTTONCONTROL_MIN_HEIGHT
            - ButtonControlTP.Companion.BUTTONCONTROL_BOTTOM_TOP_BUDGET)
    private val fastForward: JButton
    private val fastBackward: JButton
    private val toEnd: JButton
    private val toStart: JButton

    /// TimeControl Config END
    init {
        preferredSize = Dimension(TopPane.Companion.TOPPANE_MIN_WIDTH, TIMECONTROL_HEIGHT / 4)
        minimumSize = preferredSize
        isOpaque = false
        layout = GridLayout(1, 4)
        fastForward = JButton()
        fastForward.layout = BorderLayout()
        fastForward.icon = resizeImage(
            Global.rd.getFromAsImageIcon(FAST_FORWARD_ICON),
            ICON_SIZING,
            ICON_SIZING
        )
        fastForward.preferredSize = Dimension(
            TopPane.Companion.TOPPANE_MIN_WIDTH / 4,
            TIMECONTROL_HEIGHT / 4
        )
        fastForward.maximumSize = fastForward.preferredSize
        fastForward.border = BorderFactory.createEmptyBorder()
        fastForward.background = null
        fastForward.addActionListener(this)
        fastBackward = JButton()
        fastBackward.layout = BorderLayout()
        fastBackward.icon = resizeImage(
            Global.rd.getFromAsImageIcon(FAST_BACKWARD_ICON),
            ICON_SIZING,
            ICON_SIZING
        )
        fastBackward.preferredSize = Dimension(
            TopPane.Companion.TOPPANE_MIN_WIDTH / 4,
            TIMECONTROL_HEIGHT / 4
        )
        fastBackward.maximumSize = fastBackward.preferredSize
        fastBackward.border = BorderFactory.createEmptyBorder()
        fastBackward.background = null
        fastBackward.addActionListener(this)
        toEnd = JButton()
        toEnd.icon = resizeImage(
            Global.rd.getFromAsImageIcon(TO_END_ICON),
            ICON_SIZING,
            ICON_SIZING
        )
        toEnd.preferredSize = Dimension(
            TopPane.Companion.TOPPANE_MIN_WIDTH / 4,
            TIMECONTROL_HEIGHT / 4
        )
        toEnd.maximumSize = toEnd.preferredSize
        toEnd.border = BorderFactory.createEmptyBorder()
        toEnd.background = null
        toEnd.addActionListener(this)
        toStart = JButton()
        toStart.icon = resizeImage(
            Global.rd.getFromAsImageIcon(TO_START_ICON),
            ICON_SIZING,
            ICON_SIZING
        )
        toStart.preferredSize = Dimension(
            TopPane.Companion.TOPPANE_MIN_WIDTH / 4,
            TIMECONTROL_HEIGHT / 4
        )
        toStart.maximumSize = toStart.preferredSize
        toStart.border = BorderFactory.createEmptyBorder()
        toStart.background = null
    ...
    override fun actionPerformed(arg0: ActionEvent) {
        if (arg0.source == toEnd) {
            info("> Submitting time control for [toEnd]")
        } else if (arg0.source == toStart) {
            info("> Submitting time control for [toStart]")
        } else if (arg0.source == fastBackward) {
            info("> Submitting time control for [fastBackward]")
            Global.player.stream!!.seekTo(-INTERVAL_TIMING_MILLISECONDS)
        } else if (arg0.source == fastForward) {
            info("> Submitting time control for [fastForward]")
            Global.player.stream!!.seekTo(INTERVAL_TIMING_MILLISECONDS)
        }
    }

    companion object {
        var INTERVAL_TIMING_MILLISECONDS = 10000L
    }
}