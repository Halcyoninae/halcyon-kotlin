 ...
 * @since 3.1
 */
class LikeList constructor() : FileList(
    VirtualFolder("Liked", *fetchLikedTracks()),
    Global.rd.getFromAsImageIcon(FileList.Companion.FILEVIEW_ICON_FOLDER_CLOSED),
    Global.rd.getFromAsImageIcon(FileList.Companion.FILEVIEW_ICON_FOLDER_OPEN),
    Global.rd.getFromAsImageIcon(FileList.Companion.FILEVIEW_ICON_LIKED_FILE), "Unlike", itemListener
) {
    /**
     * Returns the virtual folder representing
     * this viewport.
     *
     * @return [com.halcyoninae.halcyon.filesystem.VirtualFolder]
     */
    @Transient
     ...
     */
    init {
        folder = getFolderInfo() as VirtualFolder?
    }
     ...
     */
    fun unset(file: String?) {
        try {
            warn("Requesting Unset: " + file)
            for (f: File in getFileMap().keys) {
                if ((f.getAbsolutePath() == file)) {
                    getFileMap().remove(f)
                    folder!!.removeFile(f)
                    info<VirtualFolder?>(folder)
                    val model: DefaultTreeModel = getTree().getModel()
                    model.removeNodeFromParent(getFileMap().get(f))
                    model.reload()
                    good("Checking FileMap: " + getFileMap().get(f))
                    warn(folder.removeFile(f))
                    warn("UNSET: " + f.getAbsolutePath())
                    break
                }
            }
     ...
     */
    fun set(file: String?) {
        warn("LIKED_LIST_SET: " + folder!!.asListFiles.contains(File(file)))
        if (!folder.asListFiles.contains(File(file))) {
            val node: DefaultMutableTreeNode = DefaultMutableTreeNode(File(file).getName())
            getFileMap().put(File(file), node)
            super.getRoot().add(node)
            getTree().revalidate()
            folder.addFile(File(file))
            (getTree().getModel() as DefaultTreeModel).reload()
            info<VirtualFolder?>(folder)
        }
    }

    public override fun revalidateFiles() {
        val toRemove: MutableList<File> = ArrayList()
        for (f: File in getFileMap().keys) {
            if (!f.exists() || !f.isFile()) {
                (getTree().getModel() as DefaultTreeModel).removeNodeFromParent(getFileMap().get(f))
                toRemove.add(f)
                warn("File not found: " + f.getName())
            }
        }
        for (f: File? in toRemove) {
            getFileMap().remove(f)
        }
    }
     ...
     */
    fun isLiked(file: String?): Boolean {
        return Arrays.asList(*folder!!.files).contains(File(file))
    }

    override val rootNameTree: String
        get() {
            return "Liked"
        }

    companion object {
        private val itemListener: RightClickHideItemListener = object : RightClickHideItemListener {
            public override fun onRemove(content: String?) {
