 ...
 * @since 3.0
 */
class RefreshFileView : JButton(
    ImageIcon(
        resize(
            imageIconToBI(Global.rd.getFromAsImageIcon(Manager.BBLOC_REFRESH_FILEVIEW_ICON)),
            16,
            16
        )
    )
    ...
    init {
        rolloverIcon = ImageIcon(
            resize(
                imageIconToBI(Global.rd.getFromAsImageIcon(Manager.BBLOC_REFRESH_FILEVIEW_ICON_PRESSED)),
                16,
                16
            )
        )
        toolTipText = Manager.REFRESH_BUTTON_TOOLTIP
        isOpaque = true
        background = null
        border = null
