 ...
 * @since 3.0
 */
class Settings : JButton(), BBlocButton {
    private val e = SettingsPane()

    init {
        rolloverIcon = resizeImage(
            Global.rd.getFromAsImageIcon(Manager.SETTINGS_BUTTON_PRESSED_ICON),
            16,
            16
        )
        icon = resizeImage(
            Global.rd.getFromAsImageIcon(Manager.SETTINGS_BUTTON_DEFAULT_ICON),
            16,
            16
        )
        toolTipText = Manager.SETTINGS_BUTTON_TOOLTIP
        isOpaque = true
        border = null
