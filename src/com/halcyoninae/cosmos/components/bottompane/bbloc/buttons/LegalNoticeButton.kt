 ...
 * [com.halcyoninae.cosmos.components.info.InformationDialog] which displays
 * information regarding legal documentation for all libraries and this program.
 *
 * @author Jack Meng
 * @since 3.1
 */
class LegalNoticeButton : JButton(), BBlocButton {
    val LEGALNOTICEBBLOC_ICON_BUTTON_NORMAL = Manager.RSC_FOLDER_NAME + "/bbloc/legals_normal.png"
    val LEGALNOTICEBBLOC_ICON_BUTTON_PRESSED = Manager.RSC_FOLDER_NAME + "/bbloc/legals_pressed.png"
    val LEGALNOTICEDIALOG_SCROLL_PANE_MIN_HEIGHT = LEGALNOTICEDIALOG_MIN_HEIGHT - 100

    /// LEGALNOTICEDIALOG Config END
    init {
        icon = resizeImage(
            Global.rd.getFromAsImageIcon(LEGALNOTICEBBLOC_ICON_BUTTON_NORMAL), 16, 16
        )
        rolloverIcon = resizeImage(
            Global.rd.getFromAsImageIcon(LEGALNOTICEBBLOC_ICON_BUTTON_PRESSED), 16, 16
        )
    ...
    companion object {
        ...
        const val LEGALNOTICEDIALOG_MIN_WIDTH = 500
        const val LEGALNOTICEDIALOG_MIN_HEIGHT = 550
        const val LEGAL_NOTICE_DOCS = Manager.RSC_FOLDER_NAME + "/files/LICENSE.html"
        const val LEGAL_NOTICE_PROPERTIES_DOCS = Manager.RSC_FOLDER_NAME + "/files/PROPERTIES_FILE.html"
    }
}