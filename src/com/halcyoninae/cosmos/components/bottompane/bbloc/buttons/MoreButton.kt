 ...
 * @since 3.3
 */
class MoreButton : JButton(), BBlocButton {
    // MoreButton CONFIG END
    private var isPressed: Boolean

    init {
        icon = resizeImage(Global.rd.getFromAsImageIcon(DEFAULT_ICON), 16, 16)
        rolloverIcon = resizeImage(Global.rd.getFromAsImageIcon(PRESSED_ICON), 16, 16)
        toolTipText = "Open a window to display more apps"
        isOpaque = true
        border = null
        ...
        isPressed = false
        Global.moreApps.addWindowListener(object : WindowAdapter() {
    ...
    override fun actionPerformed(e: ActionEvent) {
        if (!isPressed) {
            ...
            Global.moreApps.isVisible = false
            isPressed = false
        }
    }
    ...
    companion object {
        ...
        const val DEFAULT_ICON = Manager.RSC_FOLDER_NAME + "/bbloc/dots.png"
        const val PRESSED_ICON = Manager.RSC_FOLDER_NAME + "/bbloc/dots_pressed.png"
    }
}