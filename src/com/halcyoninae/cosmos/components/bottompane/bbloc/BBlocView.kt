 ...
 * @since 3.0
 */
class BBlocView : JPanel() {
    /// BBloc Config START
    /// B Bloc Size Config START ///
    val B_MIN_WIDTH: Int = Manager.MIN_WIDTH - FileList.Companion.FILEVIEW_MIN_WIDTH - 40
    val B_MIN_HEIGHT = Manager.MIN_HEIGHT / 2
    val B_MAX_WIDTH: Int = B_MIN_WIDTH - FileList.Companion.FILEVIEW_MAX_WIDTH
    val B_MAX_HEIGHT = Manager.MAX_HEIGHT / 2

    /// B Bloc Size Config END ///
    /// BBloc Config END
    init {
        preferredSize = Dimension(B_MIN_WIDTH, B_MIN_HEIGHT)
        minimumSize = Dimension(B_MIN_WIDTH, B_MIN_HEIGHT)
    }

    /**
     * @param buttons
     */
    fun addBBlockButtons(vararg buttons: BBlocButton) {
        for (b in buttons) {
            add(b.component)
        }
    }
