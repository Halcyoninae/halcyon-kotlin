 ...
 * @since 3.0
 */
class BottomPane constructor() : JTabbedPane() {
    private val tabs: MutableList<FileList>

    /**
     * Holds a list of folder paths in correspondence with their index
     * in the JTabbedPane.
     */
    private val tabsMap: MutableMap<String, Int>
    ...
    init {
        tabsMap = HashMap()
        setPreferredSize(Dimension(FileList.Companion.FILEVIEW_MAX_WIDTH, FileList.Companion.FILEVIEW_MIN_HEIGHT))
        setMinimumSize(Dimension(FileList.Companion.FILEVIEW_MIN_WIDTH, FileList.Companion.FILEVIEW_MIN_HEIGHT))
        tabs = ArrayList()
     ...
     */
    fun findByTree(tree: JTree): FileList? {
        for (tab: FileList in tabs) {
            if ((tab.getTree() == tree)) {
                return tab
            }
        }
     ...
     */
    fun containsFolder(folderAbsoluteStr: String?): Boolean {
        return Program.cacher.savedPlaylists.contains(folderAbsoluteStr)
    }
     ...
     */
    fun pokeNewFileListTab(plain: FileList) {
        add(plain.getFolderInfo().name, plain)
        tabs.add(plain)
    }
     ...
     */
    fun pokeNewFileListTab(folder: String) {
        val list: FileList = FileList(PhysicalFolder(folder))
        Program.cacher.savedPlaylists.add(File(folder).getAbsolutePath())
        add(File(folder).getName(), list)
        val button: TabButton = TabButton(this)
        setTabComponentAt(getTabCount() - 1, button)
        setToolTipTextAt(getTabCount() - 1, folder)
        tabsMap.put(folder, getTabCount() - 1)
        button.setListener(object : RemoveTabListener {
            public override fun onRemoveTab() {
                warn("Removing tab > " + folder)
                val i: Int = (tabsMap.get(folder))!!
                Program.cacher.savedPlaylists.remove(folder)
    ...
    @Synchronized
    fun mastRevalidate() {
        val needToRemove: MutableList<Int> = ArrayList()
        val i: Int = 0
        for (l: FileList in tabs) {
            if (!l.isVirtual) {
                if ((!File(l.getFolderInfo().absolutePath).exists()
                            || !File(l.getFolderInfo().absolutePath).isDirectory())
                ) {
                    removeTabAt(i)
                    needToRemove.add(i)
                    Program.cacher.savedPlaylists.remove(l.getFolderInfo().absolutePath)
                    tabsMap.remove(l.getFolderInfo().absolutePath)
                } else {
                    l.revalidateFiles()
                }
            }
        }
        for (ix: Int in needToRemove) {
            tabs.removeAt(ix)
        }
    }
}