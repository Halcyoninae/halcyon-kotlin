 ...
 * @since 3.1
 */
class SettingsPane : JFrame(), Runnable, ActionListener {
    private val pane: JTabbedPane
    private val aPane: JPanel
    private val buttons: JPanel
    private val close: JButton
    private val ok: JButton
    private val apply: JButton

    init {
        preferredSize = Dimension(Manager.SETTINGS_MIN_WIDTH, Manager.SETTINGS_MIN_HEIGHT + 60)
        defaultCloseOperation = DISPOSE_ON_CLOSE
        isFocusable = false
        minimumSize = preferredSize
        title = "Halcyon Settings ~ exoad"
        iconImage = Global.rd.getFromAsImageIcon(Manager.PROGRAM_ICON_LOGO).image
        pane = JTabbedPane()
        pane.tabLayoutPolicy = JTabbedPane.WRAP_TAB_LAYOUT
        pane.preferredSize = Dimension(Manager.SETTINGS_MIN_WIDTH, Manager.SETTINGS_MIN_HEIGHT)
        pane.minimumSize = pane.preferredSize
        pane.border = BorderFactory.createLineBorder(ColorManager.BORDER_THEME)
        for (tab in tabs) {
            pane.addTab(tab.tabName, null, tab.tabContent, tab.tabToolTip)
        }
        buttons = JPanel()
        buttons.preferredSize = Dimension(Manager.SETTINGS_MIN_WIDTH, 30)
        ...
        buttons.add(apply)
        aPane = JPanel()
        aPane.preferredSize = preferredSize
        aPane.minimumSize = minimumSize
        aPane.layout = BorderLayout()
    ...
    override fun actionPerformed(e: ActionEvent) {
        if (e.source == close) {
            ...
            dispose()
        } else if (e.source == apply) {
            ExternalResource.pm.save()
        }
    }

    companion object {
        protected val tabs = arrayOf<SettingsTabs>(
            AudioSettings()
        )
    }
}