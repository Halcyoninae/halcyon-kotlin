...
object SettingsUtil {
     ...
     */
    fun getPropertyAsComponent(property: Property, dim: Dimension?): JPanel {
        val panel = JPanel()
        panel.layout = FlowLayout(FlowLayout.LEFT)
        val label = JLabel(property.propertyName)
        if (property.pr != null) {
            val comboBox = JComboBox<String>()
            if (property.commonProperties != null) {
                for (allowedProperty in property.commonProperties) {
                    comboBox.addItem(allowedProperty)
                }
            }
            comboBox.selectedItem = property.defaultProperty
            panel.add(label)
            panel.add(comboBox)
        } else {
            val textField = JTextField(property.defaultProperty)
            panel.add(label)
            panel.add(textField)
        }
        return panel
