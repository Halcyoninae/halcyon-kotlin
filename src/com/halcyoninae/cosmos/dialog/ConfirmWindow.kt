...
class ConfirmWindow(content: String?, vararg listeners: ConfirmationListener) : JFrame(DIALOG_CONFIRM_WIN_TITLE),
    Runnable, ActionListener {
    /// Confirm Config START
    val DIALOG_CONFIRM_MIN_WIDTH = 300
    val DIALOG_CONFIRM_MIN_HEIGHT = 200
    val DIALOG_CONFIRM_PROMPT_AREA_MIN_WIDTH = DIALOG_CONFIRM_MIN_WIDTH - 20
    val DIALOG_CONFIRM_PROMPT_AREA_MIN_HEIGHT = DIALOG_CONFIRM_MIN_HEIGHT / 5

    /// Confirm Config END
    private val confirm: JButton

    @Transient
    private val listeners: Array<ConfirmationListener>

    init {
        iconImage = Global.rd.getFromAsImageIcon(Manager.PROGRAM_ICON_LOGO).image
        this.listeners = listeners
        preferredSize = Dimension(DIALOG_CONFIRM_MIN_WIDTH, DIALOG_CONFIRM_MIN_HEIGHT)
        setLocationRelativeTo(null)
        defaultCloseOperation = DISPOSE_ON_CLOSE
        confirm = JButton("Confirm")
        confirm.alignmentY = CENTER_ALIGNMENT
        confirm.addActionListener(this)
        val cancel = JButton("Cancel")
        cancel.alignmentY = CENTER_ALIGNMENT
        cancel.addActionListener(this)
        val prompt = JTextArea(content)
        prompt.lineWrap = true
        prompt.wrapStyleWord = true
        prompt.alignmentY = CENTER_ALIGNMENT
        prompt.isEditable = false
        val container = JScrollPane(prompt)
        container.preferredSize = Dimension(DIALOG_CONFIRM_PROMPT_AREA_MIN_WIDTH, DIALOG_CONFIRM_PROMPT_AREA_MIN_HEIGHT)
        container.verticalScrollBarPolicy = ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS
        container.horizontalScrollBarPolicy = ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER
        val jp = JPanel()
        jp.layout = FlowLayout(FlowLayout.CENTER)
        jp.preferredSize = Dimension(DIALOG_CONFIRM_PROMPT_AREA_MIN_WIDTH, DIALOG_CONFIRM_PROMPT_AREA_MIN_HEIGHT)
        jp.add(confirm)
        jp.add(cancel)
        layout = BorderLayout()
        isResizable = false
     ...
     * @param status
     */
    private fun dispatchConfirmationEvents(status: Boolean) {
    ...
    override fun actionPerformed(e: ActionEvent) {
        dispatchConfirmationEvents(e.source == confirm)
        dispose()
    }
    ...
    companion object {
        const val DIALOG_CONFIRM_WIN_TITLE = "Confirmation!"
    }
}