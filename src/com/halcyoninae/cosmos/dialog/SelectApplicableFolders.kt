 ...
 * @since 3.0
 */
class SelectApplicableFolders : JFileChooser(), Runnable {
    @Transient
    private var fs: FolderSelectedListener? = null
    ...
    init {
        ...
        approveButtonText = "Add Folder"
        approveButtonToolTipText = "Add this folder as a playlist to play from"
        dialogTitle = "Halcyon ~ Add Playlist"
        isFileHidingEnabled = false
        isMultiSelectionEnabled = false
        val fsv = FileSystemView.getFileSystemView()
        currentDirectory = fsv.homeDirectory
     ...
     */
    fun setFolderSelectedListener(fs: FolderSelectedListener?) {
        this.fs = fs
    }
    ...
    override fun run() {
        SwingUtilities.invokeLater {
            val returnVal = showOpenDialog(FSVDefault())
            if (returnVal == APPROVE_OPTION && fs != null) {
                fs!!.folderSelected(selectedFile.absolutePath)
            }
        }
    }
    ...
    interface FolderSelectedListener {
         ...
         */
        fun folderSelected(folder: String)
    }
}