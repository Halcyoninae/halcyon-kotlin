...
class SlidersDialog : JFrame(), Runnable, StatusUpdateListener {
    ...
    init {
        pane = JTabbedPane()
        val basicSlidersPane = JPanel()
        basicSlidersPane.layout = FlowLayout(FlowLayout.LEADING, 10, 15)
        val balanceSlider = JSlider(SwingConstants.VERTICAL, 0, 100, bal)
        balanceSlider.paintTicks = true
        balanceSlider.snapToTicks = true
        balanceSlider.toolTipText = "Balance Slider: " + balanceSlider.value
        balanceSlider.addChangeListener {
            if (Global.player.stream!!.controls != null) {
                Global.player.stream!!.setBalance(balanceSlider.value.toFloat())
            }
            balanceSlider.toolTipText = "Balance Slider: " + balanceSlider.value
            bal = balanceSlider.value
        }
        val panSlider = JSlider(SwingConstants.VERTICAL, 0, 100, pan)
        panSlider.majorTickSpacing = 10
        panSlider.minorTickSpacing = 100
        panSlider.snapToTicks = true
        panSlider.paintTicks = true
        panSlider.toolTipText = "Pan Slider: " + panSlider.value
        panSlider.addChangeListener {
            if (Global.player.stream!!.controls != null) {
                Global.player.stream!!.setPan(panSlider.value.toFloat())
            }
            panSlider.toolTipText = ("Pan Slider: " + panSlider.value + " | "
                    + if (balanceSlider.value > 50) "RIGHT" else if (balanceSlider.value < 50) "LEFT" else "CENTER")
            pan = panSlider.value
        }
        ...
        title = "Sliders"
        preferredSize = Dimension(300, 350)
    ...
    override fun statusUpdate(status: TailwindStatus?) {
        if (status == TailwindStatus.OPEN) {
            Global.player.stream!!.setBalance(bal.toFloat())
            Global.player.stream!!.setPan(pan.toFloat())
        }
    }
}