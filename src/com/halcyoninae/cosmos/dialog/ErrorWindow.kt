 ...
 * @since 3.0
 */
class ErrorWindow(content: String?) : JFrame(DIALOG_ERROR_WIN_TITLE), Runnable {
    /// Error Window Config START
    val DIALOG_ERROR_MIN_WIDTH = 300
    val DIALOG_ERROR_MIN_HEIGHT = 150

    /// Error Window Config END
    init {
        iconImage =
            Global.rd.getFromAsImageIcon(Manager.PROGRAM_ICON_LOGO).image
        preferredSize = Dimension(DIALOG_ERROR_MIN_WIDTH, DIALOG_ERROR_MIN_HEIGHT)
        setLocationRelativeTo(null)
        defaultCloseOperation = DISPOSE_ON_CLOSE
        isResizable = false
        val jt = JTextArea(content)
        jt.isEditable = false
        val scrollPane = JScrollPane(jt)
        scrollPane.preferredSize = Dimension(DIALOG_ERROR_MIN_WIDTH, DIALOG_ERROR_MIN_HEIGHT)
    ...
    companion object {
        const val DIALOG_ERROR_WIN_TITLE = "Exception!"
    }
}