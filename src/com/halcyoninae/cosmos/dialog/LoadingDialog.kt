 ...
 * @since 3.1
 */
class LoadingDialog(text: String?, isImportant: Boolean) : JFrame() {
    private val bar: JProgressBar

    init {
        preferredSize = Dimension(400, 200)
        isResizable = false
        isAlwaysOnTop = isImportant
        ...
        panel.add(label)
        bar = JProgressBar()
        bar.preferredSize = Dimension(400, 20)
        bar.isStringPainted = true
        bar.string = "Loading..."
        bar.alignmentX = CENTER_ALIGNMENT
     ...
     */
    @JvmOverloads
    fun run(task: Runnable = Runnable {}) {
        pack()
