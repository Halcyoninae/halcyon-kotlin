...
object ThemeBundles {
     ...
     */
    fun searchFor(canonicalName: String): Theme {
        for (theme in themes) {
            if (theme.canonicalName == canonicalName) {
                return theme
            }
        }
        return DarkGreen()
     ...
     */
    val canonicNames: Array<String?>
        get() {
            val canonicNames = arrayOfNulls<String>(themes.size)
            for (i in themes.indices) canonicNames[i] = themes[i].canonicalName
            return canonicNames
        }
}