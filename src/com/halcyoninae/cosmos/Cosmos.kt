 ...
 * @since 3.0
 */
class Cosmos constructor(mainPane: JSplitPane) : Runnable, StatusUpdateListener {
    /**
    ...
    init {
        ...
        frame = JFrame("Halcyon Beta ~ exoad")
        if ((ExternalResource.pm.get(ProgramResourceManager.KEY_PROGRAM_FORCE_OPTIMIZATION) == "false")) {
            frame.setUndecorated(true)
            frame = object : JFrame("Halcyon Beta ~ exoad") {
                public override fun paint(g: Graphics) {
                    super.paint(g)
                    val g2d: Graphics2D = g as Graphics2D
                    g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON)
                    g2d.setColor(Color.BLACK)
                    g2d.drawRoundRect(0, 0, frame.getWidth(), frame.getHeight(), 20, 20)
                }
            }
        }
        frame.setIconImage(Global.rd.getFromAsImageIcon(Manager.PROGRAM_ICON_LOGO).getImage())
        frame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE)
        frame.setPreferredSize(Dimension(Manager.MIN_WIDTH, Manager.MIN_HEIGHT))
        frame.setMinimumSize(frame.getPreferredSize())
        frame.getContentPane().add(mainPane)
        frame.addWindowListener(InstantClose())
    }
     ...
     * @param status
     */
    public override fun statusUpdate(status: TailwindStatus?) {
        if ((status == TailwindStatus.PLAYING)) {
            frame.setIconImage(Global.rd.getFromAsImageIcon(Manager.PROGRAM_GREEN_LOGO).getImage())
        } else if ((status == TailwindStatus.OPEN)) {
            frame.setIconImage(Global.rd.getFromAsImageIcon(Manager.PROGRAM_BLUE_LOGO).getImage())
        } else if ((status == TailwindStatus.PAUSED)) {
            frame.setIconImage(Global.rd.getFromAsImageIcon(Manager.PROGRAM_RED_LOGO).getImage())
        } else {
            frame.setIconImage(Global.rd.getFromAsImageIcon(Manager.PROGRAM_ICON_LOGO).getImage())
        }
    }
    ...
    companion object {
        ...
        fun refreshUI(theme: Theme) {
            try {
                UIManager.setLookAndFeel(theme.getLAF().getName())
            } catch (ex: ClassNotFoundException) {
                ex.printStackTrace()
                ErrorWindow(ex.message).run()
                dispatchLog(ex)
            ...
            for (frame: Window in Frame.getFrames()) {
                try {
                    SwingUtilities.updateComponentTreeUI(frame)
                } catch (ex: NullPointerException) {
                    // Do nothing bc sometimes swing is bad...
                }
                for (e: Component in getAllComponents(frame)) {
                    if (e is JLabel) {
