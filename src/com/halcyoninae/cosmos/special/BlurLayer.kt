 ...
 * @since 3.3
 */
class BlurLayer : LayerUI<JComponent?>() {
    @Transient
    private var bufferImage: BufferedImage? = null

    @Transient
    private val image: BufferedImageOp

    init {
        image = ConvolveOp(Kernel(3, 3, CloudSpinFilters.BLUR_KERNEL), ConvolveOp.EDGE_NO_OP, null)
    }
    ...
    override fun paint(g: Graphics, c: JComponent) {
        ...
        if (bufferImage == null || bufferImage!!.width != w || bufferImage!!.height != h) {
            bufferImage = BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB)
        }
        val g2 = bufferImage!!.createGraphics()
        g2.clip = g.clip
        super.paint(g2, c)
        g2.dispose()
        val g2_2 = g as Graphics2D
