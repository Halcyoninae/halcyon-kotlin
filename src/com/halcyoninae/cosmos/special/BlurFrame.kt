 ...
 * @since 3.3
 */
class BlurFrame(size: Dimension?) : JFrame() {
    private val contentPanelDefault: JPanel

    init {
        val layerUI: LayerUI<JComponent> = BlurLayer()
        contentPanelDefault = JPanel()
        contentPanelDefault.preferredSize = size
        val jlayer = JLayer(contentPanelDefault, layerUI)
        add(jlayer)
        preferredSize = size
        setSize(size)
    }
