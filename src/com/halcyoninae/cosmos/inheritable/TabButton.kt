...
class TabButton constructor(private val parentPane: JTabbedPane) : JPanel() {
    @Transient
    private var listener: RemoveTabListener? = null
    ...
    init {
        ...
        val label: JLabel = object : JLabel() {
            public override fun getText(): String {
                val i: Int = parentPane.indexOfTabComponent(this@TabButton)
                if (i != -1) {
     ...
     */
    val internalButton: CloseTabButton
        get() {
            return getComponent(1) as CloseTabButton
        }

    /**
     * @param listener
     */
    fun setListener(listener: RemoveTabListener?) {
        this.listener = listener
    }

    fun dispatchRemoveEvent() {
        if (listener != null) {
            listener!!.onRemoveTab()
        }
    }

    open interface RemoveTabListener {
        fun onRemoveTab()
    }

    inner class CloseTabButton constructor() : JButton(), ActionListener {
        init {
            setPreferredSize(Dimension(Manager.BUTTON_STD_ICON_WIDTH_N_HEIGHT, Manager.BUTTON_STD_ICON_WIDTH_N_HEIGHT))
