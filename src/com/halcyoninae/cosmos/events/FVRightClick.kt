 ...
 * @since 3.0
 */
class FVRightClick : MouseAdapter {
    private var tree: TabTree? = null
    private var lastJTree: JTree? = null
    private var rightClicks = 0
    private var hideString = "Hide Item"
    private var hideTask: RightClickHideItemListener? = null

    constructor(tree: TabTree?, hideItemString: String, hideTask: RightClickHideItemListener?) : this(tree) {
        this.hideTask = hideTask
        hideString = hideItemString
    }

    constructor(tree: TabTree?) {
        this.tree = tree
    }
    ...
    private fun popup(e: MouseEvent) {
        ...
        val rcNode = path.lastPathComponent as DefaultMutableTreeNode
        val s = t.selectionPaths
        ...
        var refreshMenuItem: JMenuItem? = null
        if (rcNode != t.model.root) {
            refreshMenuItem = JMenuItem(hideString)
            refreshMenuItem.addActionListener(ActionListener { ev: ActionEvent? ->
                try {
                    val parent = rcNode.parent as DefaultMutableTreeNode
                    parent.remove(rcNode)
                    val model = t.model as DefaultTreeModel
                    model.reload()
                    tree!!.remove(rcNode.toString())
                    info("Removing Physical: " + tree!!.getSelectedNode(rcNode))
                    hideTask!!.onRemove(tree!!.getSelectedNode(rcNode))
                    if (hideTask == null) Program.cacher.pingExcludedTracks(tree.getPath() + "/" + rcNode)
                } catch (excec: NullPointerException) {
                    // IGNORED
                }
            })
        }
        var sortAZ: JMenuItem? = null
        var sortZA: JMenuItem? = null
        var sortShuffle: JMenuItem? = null
        if (rcNode == t.model.root) {
            sortAZ = JMenuItem("Sort A-Z")
            sortAZ.addActionListener(ActionListener { ev: ActionEvent? -> tree!!.sort(TabTreeSortMethod.ALPHABETICAL) })
            sortZA = JMenuItem("Sort Z-A")
            sortZA.addActionListener(ActionListener { ev: ActionEvent? -> tree!!.sort(TabTreeSortMethod.REV_ALPHABETICAL) })
            sortShuffle = JMenuItem("Sort Shuffle")
            sortShuffle.addActionListener(ActionListener { ev: ActionEvent? -> tree!!.sort(TabTreeSortMethod.SHUFFLE) })
        }
        val audioInfoItem = JMenuItem("Information")
        audioInfoItem.addActionListener { ev: ActionEvent? ->
            try {
                if (rcNode != t.model.root) {
                    Thread { AudioInfoDialog(AudioInfo(tree!!.getSelectedNode(rcNode))).run() }.start()
                } else {
                    StraightTextDialog(
                        "<html><body><p><strong>Folder:</strong> " + tree.getPath() + "</p></body></html>"
                    ).run()
                }
            } catch (excec: NullPointerException) {
                ErrorWindow("A root node is not a valid audio stream.").run()
            }
        }
        if (refreshMenuItem != null) popup.add(refreshMenuItem)
        if (sortAZ != null) {
            popup.add(sortAZ)
            popup.add(sortZA)
    ...
    override fun mouseClicked(e: MouseEvent) {
        if (e.button == MouseEvent.BUTTON1) {
            if (lastJTree == null) {
                lastJTree = e.source as JTree
            }
            if (lastJTree != e.source) {
                if (rightClicks == 2) {
                    val pathTree = e.source as JTree
                    val path = pathTree.selectionPath
                    if (path != null) {
                        val node = path.lastPathComponent as DefaultMutableTreeNode
                        if (node.parent.toString() != StringManager.JTREE_ROOT_NAME) {
                            var f: File? = null
                            if (tree!!.isVirtual) {
                                f = File(node.toString())
                            } else {
                                f = File(
                                    Global.bp.findByTree(e.source as JTree)
                                        .folderInfo
                                        .absolutePath +
    ...
    interface RightClickHideItemListener {
