 ...
 * @since 3.0
 */
class ForceDividerLocation(private val e: JSplitPane, threshold: Int) : PropertyChangeListener {
    private val min: Int
    private val max: Int
     ...
     */
    init {
        min = (e.preferredSize.getHeight() / 2 - threshold).toInt()
        max = (threshold + e.preferredSize.getHeight() / 2).toInt()
    }
    ...
    override fun propertyChange(evt: PropertyChangeEvent) {
        if (evt.propertyName == "dividerLocation") {
            val dividerLoc = e.dividerLocation
            if (dividerLoc < min) {
                e.dividerLocation = min
            }
            if (dividerLoc > max) {
                e.dividerLocation = max
            }
        }
    }
}